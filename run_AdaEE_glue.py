...
def train(args, train_dataset, model, tokenizer,  kd_loss="kd"):
    ...
    for i in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if type(loss) == tuple:
                tea_loss = loss[3]
                multi_loss = loss[1] + loss[2]
                full_loss = loss[0]

            if args.n_gpu > 1:
                tea_loss = tea_loss.mean()
                multi_loss = multi_loss.mean()
                full_loss = full_loss.mean()

            if args.gradient_accumulation_steps > 1:
                tea_loss = tea_loss / args.gradient_accumulation_steps
...
def main():
    ...
    # kd hyperparameters
    parser.add_argument('--gamma', type=float, default=0.1, help='gamma for kld loss')
